...
Example call:
python3 my_scripts/compute_flexwer.py -r /decode/scoring_kaldi/test_filt.txt -h /decode/scoring_kaldi/penalty_0.0/11.txt -m corpus_data/norm2dieth.json --verbose
"""
...
def set_args():
    ...
    parser.add_argument('-m', '--n2d_mapping', required=False, help='If provided, flexible WER is calculated based on forms found in mapping.')
...
def opcodes(source, target, n2d_map=None, d2n_map=None):
    ...
    source=reference, target=hypothesis
    """
    n = len(source)
    m = len(target)
...
def main(args=None):
    ...
    if use_args is True:
        ...
        with open(args.ref, 'r', encoding='utf8') as r_file, open(args.hyp, 'r', encoding='utf8') as h_file:
            ...
            try:
                assert len(all_refs) == len(all_hyps)
            except AssertionError:
                print(len(all_refs))
                print(len(all_hyps))

            for i in range(len(all_refs)):
                ref = normalise_line(all_refs[i].strip())
                hyp = normalise_line(all_hyps[i].strip())
        ...
        if args.n2d_mapping:
            final_flexwer = '%{} {:.2f} [ {} / {}, {} ins, {} del, {} sub ] {}'.format('FLEXWER',
                                                                            error_rate,
                                                                            op_count,
            ...
            # print(final_flexwer)
            return final_flexwer

        else:
            final_wer = '%{} {:.2f} [ {} / {}, {} ins, {} del, {} sub ] {}'.format('FLEXWER',
                                                                             error_rate,
                                                                             op_count,
                                                                             sum(total_ops.values(
                                                                             )),
        ...
        ref_path = "/Users/inigma/Documents/UZH_Master/MasterThesis/results/phon_recog/nnet_discr/scoring_kaldi/test_filt.txt"
        hyp_path = "/Users/inigma/Documents/UZH_Master/MasterThesis/results/phon_recog/nnet_discr/scoring_kaldi/penalty_0.0/10.txt"

        n2d_mapping = "/Users/inigma/Documents/UZH_Master/MasterThesis/KALDI/kaldi_wrk_dir/data/corpus_data/norm2dieth.json"
        # n2d_mapping = None
        verbose = False

        if n2d_mapping:
            n2d_map, d2n_map = get_mappings(n2d_mapping)
        else:
            n2d_map, d2n_map = None, None

        total_ops = Counter()

        with open(ref_path, 'r', encoding='utf8') as r_file, open(hyp_path, 'r', encoding='utf8') as h_file:
            all_refs = sorted(r_file.readlines())
            all_hyps = sorted(h_file.readlines())
            try:
                assert len(all_refs) == len(all_hyps)
