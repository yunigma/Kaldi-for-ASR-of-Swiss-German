...
stage=1
transform_dir=    # In this case transform-dir must be a decode direction:
                  # hence, containing transforms for the test data
# transform_dir_decode=  # dir to find fMLLR transforms.
iter=final
nj=32 # number of decoding jobs.
...
lattice_beam=10.0 # Beam we use in lattice generation.
...
if [ ! -z "$transform_dir" ]; then
  [ "$nj" -ne "`cat $transform_dir/num_jobs`" ] \
    && echo "$nj" && echo "`cat $transform_dir/num_jobs`" \
  ...
  echo "$0: feat transform with transform_dir DONE"
...
if [ $stage -le 2 ]; then
  if [ -f "$graphdir/num_pdfs" ]; then
    echo "`cat $graphdir/num_pdfs`"
    echo "`am-info --print-args=false $alignment_model | grep pdfs | awk '{print $NF}'`"
...
if [ $stage -le 3 ]; then
  echo "Estimate speaker vectors (1st pass)."
...
if [ $stage -le 4 ]; then
  echo "Estimate speaker vectors (2nd pass)."
...
# their size.
if [ $stage -le 6 ]; then
  echo "Rescore the state-level lattices with the adapted features"
