...
if [ $stage -le 2 ]; then
  ...
  $cmd --num-threads $num_threads JOB=1:$nj $dir/log/decode.JOB.log \
    ...
    $adapt_model \
    $graphdir/HCLG.fst "$pass1feats" "ark:|gzip -c > $dir/lat.tmp.JOB.gz" \
    || exit 1;
fi
##
...
if [ $stage -le 3 ]; then
  ...
  $cmd --max-jobs-run $max_fmllr_jobs JOB=1:$nj $dir/log/fmllr_pass2.JOB.log \
    lattice-determinize-pruned$thread_string \
    --acoustic-scale=$acwt \
    --beam=4.0 \
...
if [ $stage -le 4 ]; then
  ...
  $cmd --num-threads $num_threads JOB=1:$nj $dir/log/acoustic_rescore.JOB.log \
    gmm-rescore-lattice \
    $final_model "ark:gunzip -c $dir/lat.tmp.JOB.gz|" "$feats" ark:- \| \
    lattice-determinize-pruned$thread_string \
    --acoustic-scale=$acwt \
    --beam=$lattice_beam ark:- \
    "ark:|gzip -c > $dir/lat.JOB.gz" '&&' rm \
    $dir/lat.tmp.JOB.gz || exit 1;
fi
